...
const debug = require('debug')('mochelec:mocha');
...
function create(args) {
	...
	debug('reporter: %s, ui: %s', args.reporter, args.ui);
...
function run(args, cb) {
	...
	let runner = mocha.run(cb);

	args.hook.forEach((hook) => {
		debug('hook: %s', hook);
		require(hook)(mocha, runner);
	});
